...
T.describe("core.pack.ui.fidget", function()
  ...
  T.it("done marks row with the success glyph", function()
    local UI = fresh()
    local view = UI.fidget({ open_window = false })
    view:set_status("plugin-a", "fetching")
    view:done("plugin-a")
    local lines = vim.api.nvim_buf_get_lines(view.buf, 0, -1, false)
    T.truthy(lines[1]:find(Lib.icons.status.Success, 1, true))
    view:close()
  end)

  T.it("error marks row with the failure glyph and the message", function()
    local UI = fresh()
    local view = UI.fidget({ open_window = false })
    view:set_status("plugin-a", "fetching")
    view:error("plugin-a", "timeout")
    local lines = vim.api.nvim_buf_get_lines(view.buf, 0, -1, false)
    T.truthy(lines[1]:find(Lib.icons.status.Failure, 1, true) and lines[1]:match("timeout"))
    view:close()
  end)
end)
