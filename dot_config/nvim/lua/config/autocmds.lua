...
-- DEBUG ONLY: clipboard reliability logger. Remove once we've identified the
-- intermittent yy → external paste failure. Logs to /tmp/nvim-clip.log:
--   - every TextYankPost: yank content + getreg("+") + pbpaste 200ms later
--   - FocusLost / FocusGained: getreg("+") + pbpaste at the moment
-- After hitting a failure in real use, note the timestamp and inspect the
-- log around it to see whether nvim's register, the system pasteboard,
-- or both diverged from the yanked content.
do
  local log_path = "/tmp/nvim-clip.log"
  ...
  local function pbpaste()
    local p = io.popen("pbpaste")
    if not p then return "<popen-fail>" end
    local s = p:read("*a")
    p:close()
    return s or ""
  end
  au("TextYankPost", {
    ...
    callback = function()
      ...
      local op      = ev.operator or "?"
      local kind    = ev.regtype or "?"
      local yanked  = (ev.regcontents and table.concat(ev.regcontents, "\\n")) or ""
      log(string.format("YANK op=%s kind=%s yanked=%q getreg(+)=%q",
        op, kind, yanked:sub(1, 80), vim.fn.getreg("+"):sub(1, 80)))
      vim.defer_fn(function()
        log(string.format("YANK+200ms pbpaste=%q getreg(+)=%q",
          pbpaste():sub(1, 80), vim.fn.getreg("+"):sub(1, 80)))
      end, 200)
    end,
  })
  au({ "FocusLost", "FocusGained" }, {
    group = grp,
    callback = function(args)
      log(string.format("%s getreg(+)=%q pbpaste=%q",
        args.event, vim.fn.getreg("+"):sub(1, 80), pbpaste():sub(1, 80)))
    end,
  })
end
