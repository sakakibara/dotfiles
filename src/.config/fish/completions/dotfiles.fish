...
complete -c dotfiles -f -n '__fish_use_subcommand' -a cd       -d 'Change directory to the mox repo'
...
complete -c dotfiles -f -n '__fish_seen_subcommand_from edit' \
    -a '(mox status 2>/dev/null | string match -rv "^  ERROR " | string replace -r " +\\((own|disown) [0-9]+\\)\$" "" | string replace -rf "^  \S+\s+~/(.*)\$" "\$1")' \
    -d 'managed file'
