...
T.describe("lib.parsers", function()
  ...
  T.it("records filetypes deliberately left without a parser", function()
    local M = fresh()
    M.skip("xml", "svg")
    T.eq(M.skipped("xml"), true)
    T.eq(M.skipped("svg"), true)
    T.eq(M.skipped("lua"), false)
  end)

  T.it("_reset clears skipped filetypes too", function()
    local M = fresh()
    M.skip("xml")
    M._reset()
    T.eq(M.skipped("xml"), false)
  end)
