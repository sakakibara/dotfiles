...
- **Never publish benchmark results.** Do not put numeric or qualitative benchmark results or performance claims in code, comments, documentation, commit messages, reviews, releases, or other public-facing artifacts, even when measured. Reporting results privately to the user in chat is allowed. Runnable benchmark code is allowed so people can run it themselves.
