...
function Sync-Run {
    if (-not (Get-Command mox -ErrorAction SilentlyContinue)) {
        [Console]::Error.WriteLine('sync: mox not on PATH')
        exit 1
    }
    $sourceDir = $env:MOX_REPO
    if (-not $sourceDir) {
        $base = if ($env:XDG_DATA_HOME) { $env:XDG_DATA_HOME } else { Join-Path $env:USERPROFILE '.local\share' }
        $sourceDir = Join-Path $base 'mox\dotfiles'
    }
    if (-not $sourceDir) {
        [Console]::Error.WriteLine('sync: mox repo not found')
        exit 1
    }
